...
export function findRecipeSubjects(text,recipes){
 const value=String(text??'').toLowerCase(),hits=[],seen=new Set()
 for(const r of recipes){
  if(seen.has(r.subject))continue
  seen.add(r.subject)
  for(const word of new Set([r.subject,r.name,...(aliases[r.subject]??[])])){
