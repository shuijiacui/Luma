...
export interface Geometry {template:string;x:number;y:number;width:number;height:number;rotation:number;color:string;strokeWidth:number;brushKind?:Brush;sketch?:DrawingSketch;illustrationId?:string;echoPoints?:Point[];anchor?:{x:number;y:number;width:number;height:number};placement?:string;attachment?:Point;contact?:DrawingContact}
...
export function proposalBoundsPoints(p:Geometry,aspect?:number):Point[];
