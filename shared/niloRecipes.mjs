import { addAnimalRecipes } from './niloRecipesAnimals.mjs'
import { addNatureRecipes } from './niloRecipesNature.mjs'
import { addPlaceRecipes } from './niloRecipesPlaces.mjs'
import { addFoodRecipes } from './niloRecipesFood.mjs'
import { addObjectRecipes } from './niloRecipesObjects.mjs'
import { addFantasyRecipes } from './niloRecipesFantasy.mjs'
import { addRecipeExpansion } from './niloRecipeExpansion.mjs'
import { findRecipeSubjects } from './niloRecipeNames.mjs'
...
const variants=[];
function add(subject,zh,variant,label,aspect,parts,related) {
  ...
  variants.push({id:`${subject}-${variant}`,subject,name:zh,label,related,sketch:{aspect,paths},parts:ranges,source:'Luma original',minPixels:48});
}
for(let v=0;v<3;v++) {
...
export const matchingRecipeSubjects=text=>findRecipeSubjects(text,variants);
export function recipeMatchesSubject(recipe,name) {
 return matchingRecipeSubjects(name).includes(recipe.subject);
}
export function recipeCatalogue(subjects=[],recent=[]) {
  ...
  return [...variants].sort((a,b)=>Number(recent.includes(a.id))-Number(recent.includes(b.id))
    || score(b)-score(a))
    .map(({id,subject,name,label,related,colors})=>({id,subject,name,variant:label,related,colors}));
}

// Every ID and pose stays visible; verbose associations and palettes are only
// expanded for relevant families. Adding assets does not add a model round trip.
export function creativeRecipeCatalogue(subjects=[],recent=[]){
 const families=new Map();
 for(const r of recipeCatalogue(subjects,recent)){
  if(!families.has(r.subject))families.set(r.subject,{name:r.name,subject:r.subject,related:r.related,colors:r.colors,variants:[]});
  families.get(r.subject).variants.push({id:r.id,pose:r.variant});
 }
 return {
  index:[...families.values()].map(r=>`${r.name}: ${r.variants.map(v=>`${v.id}=${v.pose}`).join('; ')}`).join('\n'),
