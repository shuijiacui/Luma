...
- 共创新图案和结构变化保留为独立描画底图：SVG 使用灰色虚线，高分辨率 PNG 使用淡灰参考轮廓。保留原画，不提交模型实线。`useCompanion` 管理请求与底图版本，`companion/proposals.ts` 校验几何；底图内部不拦截画笔，只有移动和缩放手柄接收手势。清除底图不删除孩子笔迹，底图不写入作品 PNG。撤销真实对象的协助修改时可清除相关底图，避免参考与恢复后的画面错位。
- 明确的基础修改可以直接作用于真实选区：改色、移动、等比缩放、定位由画布代码原子执行，并保留孩子线条，不需要预览确认。新增、改变造型仍走参考底图；建议式问句不当成修改授权。只有 PNG 的作品不支持恢复独立对象编辑。
- 笔迹来源、引导关联和修改记录分开保存。共同修改不能用过滤 AI 操作后的拼接图冒充独立原作；实际协助修改后，“完成”只保存作品与过程，跳过自主特征分析。当前未实现过程分析报告。旧图来源未知时同样不能伪称儿童独立创作。
- `draft.ts` 管理内存状态，`draftRecovery.ts` 将可编辑文档、工具参数与独立底图写入当前标签页的 `sessionStorage`，按账号及作品隔离，刷新时显式选择恢复或丢弃。它不恢复录音、对话和待执行请求；暂存失败需提示保存／下载。正式作品通过服务端保存，游客已保存作品留在本机；这些与会话草稿不是同一持久化层。
...
- `CanvasSelection.tsx` 在选择模式捕获点选／圈选，其余时间只显示高亮、不截获画笔。`selectionGeometry.ts` 使用实际线段命中，避免仅包围框相交就误选；圈选补选，点已选笔迹取消，选择单位是整条笔迹，不能假定自由画已被分割成完整物体。
- `DrawingCanvas` 提供选区、可编辑目标、修订号与原子修改接口。选择变化立即使旧语音请求失效；`notifySelectionChanged(false)` 只取消过期解释，点击“选好了”才重放等待选区的原指令一次。Escape 取消待执行指令。后返回的请求不得覆盖新笔迹、新选区或已撤销画面。
- 投影操作区只提供带文字的“换一个”“清除底图”；移动和缩放直接使用底图手柄，不另设调整菜单或描画开关。新笔迹按孩子实际绘制保存，不根据位置推测对象来源或自动分组；页面保留 `guided` 引导曝光标记。旧文档已有的 `guidance` 关联继续兼容，完整关联对象的变换可以同步参考；单独拖动参考不会带走已有笔迹。投影 ID 随草稿恢复，不能仅因作者是孩子就当作无引导表达。
- 基础协助不重写原始点位、颜色、笔刷或层次，而是追加 `assist` 操作；协助撤销追加 `assist-revert`，保留审计历史。显示、导出和擦除遮罩必须使用一致的变换。一次多动作修改要么全部提交，要么保持原画，撤销也作为一个整体。
- 确认功能回归时至少覆盖：缺目标语音 → 补选 → 选好了仅执行一次；改色与等比缩放 → 撤销 → 保存重开；实际共同修改跳过自主分析；新笔迹不推测关联、旧关联可恢复；底图拖动不改变实线，清除和导出不带走孩子笔迹。
...
- 画布底部麦克风、Nilo 声音和连续对话按钮同排常驻，不依赖折叠声音菜单；清楚区分收音与播放状态，并提供双语文字或可访问名称。
- 默认使用浏览器识别和朗读，不需要 Python 语音服务。浏览器 TTS 优先使用同语种童声，缺少时使用轻柔候选；不通过提高已有童声音调伪装更年幼。设备未提供童声时，不能承诺一致的儿童音色。语音能力的独立开关和可选云端音色见 [Nilo API](../server/NILO_API.md)。
