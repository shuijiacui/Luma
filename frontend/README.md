...
- 共创投影是临时状态。`useCompanion` 管理请求与候选版本，`companion/proposals.ts` 校验几何，画布使用同一笔刷渲染预览与提交。确认前不得写入笔迹、下载、保存或分析。
- 笔迹文档区分儿童和 Nilo 来源。成长分析导出儿童图层，家长展示可使用合成图；旧图来源未知时不能伪称儿童独立创作。
- `draft.ts` 保存未提交的会话内草稿；正式作品通过服务端保存，游客已保存作品留在本机。不要将内存草稿、本机偏好和已保存作品视作同一类持久化。
