...
export async function exportArchive(analyses: AnalysisSummary[], token: string, childName: string, year?: number) {
  ...
  for (const item of [...selected].reverse()) {
    ...
    sections.push(`<article><h2>${escape(new Date(item.createdAt).toLocaleDateString(locale === 'en' ? 'en-US' : 'zh-CN'))}</h2>${picture}<p>${label('AI 画面描述：', 'AI picture description: ')}${escape(item.rawDescription ?? label('暂无描述', 'No description yet'))}</p>${report ? `<p>${report.kind === 'observation-v1' ? label('画面观察', 'Picture observation') : label('历史旧版报告（含未经验证的分值）', 'Legacy report (unvalidated score)')}</p><p>${escape(report.narrative ?? '')}</p><ul>${report.evidence.map(e => `<li>${escape(e.entryId)}: ${escape(t(e.plain ?? e.summary, locale))}</li>`).join('')}</ul><h3>${label('陪伴建议', 'Ideas for togetherness')}</h3><ul>${report.parentAdvice.map(a => `<li>${escape(t(a, locale))}</li>`).join('')}</ul>` : `<p>${label('尚未生成解读', 'No reading yet')}</p>`}</article>`)
  }
  const title = locale === 'en' ? `${escape(childName)} — ${year ?? 'All'} creations` : `${escape(childName)}的${year ?? '全部'}创作记录`
  const note = locale === 'en' ? `${selected.length} artworks. AI observations are not your child's own words or psychological measurements. Legacy reports are marked. Open offline and print to PDF in your browser.` : `共 ${selected.length} 幅。AI 画面观察不代表孩子原话或心理测量；历史旧版报告已标记。可离线打开，使用浏览器打印保存为 PDF。`
