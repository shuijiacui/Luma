...
export function PeriodicReportsSection({ childId, childName, token }: { childId?: string; childName: string; token?: string }) {
  ...
  if (!childId || !token) return <p className="rounded-2xl bg-white p-6">{t("正式家庭完成创作后，可以在这里查看周报与月报。")}</p>
  const current = result?.key === key ? result : null
  return <section className="space-y-5">
    <div className="flex flex-wrap items-center gap-3">
      {(['weekly', 'monthly'] as const).map(value => <Button key={value} variant={kind === value ? 'primary' : 'secondary'} aria-pressed={kind === value} onClick={() => { setKind(value); setOffset(0) }}>{lt(value === 'weekly' ? '每周摘要' : '每月报告')}</Button>)}
      <Button variant="ghost" onClick={() => { setResult(null); setRevision(n => n + 1) }}>{t("刷新报告")}</Button>
    </div>
    <p className="text-sm text-[#6b7462]">{t("按北京时间汇总已结束的自然周（周一至周日）或自然月。有作品的周期会自动生成；本周、本月的创作可先在创作记录中查看。")}</p>
    {current?.error ? <div role="alert" className="rounded-2xl bg-white p-6">{lt(current.error)}<Button onClick={() => { setResult(null); setRevision(n => n + 1) }}>{t("重试")}</Button></div>
      : !current?.page ? <p role="status">{t("正在汇总创作…")}</p>
      : <>
        {current.page.reports.length === 0 && <p className="rounded-2xl bg-white p-6">{t("还没有已结束周期的创作记录。完成第一段创作时光后，摘要会出现在这里。")}</p>}
        {current.page.reports.map(report => <article key={report.id} className="space-y-4 rounded-3xl border border-white bg-white/90 p-6 shadow-luma-card">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h3 className="text-lg font-bold">{childName} · {lt(report.summary.periodLabel)}</h3>
          ...
          <p className="text-xs leading-relaxed text-[#6b7462]">{lt(report.summary.note)} {t("下载的数据包含来源作品编号，可与成长档案核对。")}</p>
        </article>)}
        {(offset > 0 || current.page.nextOffset !== null) && <div className="flex gap-3"><Button disabled={offset === 0} onClick={() => setOffset(n => Math.max(0, n - 12))}>{t("上一页")}</Button><Button disabled={current.page.nextOffset === null} onClick={() => setOffset(current.page!.nextOffset!)}>{t("下一页")}</Button></div>}
      </>}
  </section>
}
