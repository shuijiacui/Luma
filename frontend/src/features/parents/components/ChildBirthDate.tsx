...
export function ChildBirthDate({ childId, initial, token, onSaved }: { childId: string; initial?: string | null; token: string; onSaved: () => void }) {
  ...
  return <form className="mt-4 flex flex-wrap items-center gap-3" onSubmit={async e => {
    ...
    <label className="text-sm">{t("出生日期（由家长填写）")}<input type="date" value={value} max={new Date().toISOString().slice(0, 10)} onChange={e => setValue(e.target.value)} className="rounded border p-2" /></label>
    <Button disabled={busy} type="submit">{t("保存日期")}</Button>
    <p role="status" className="text-xs">{lt(status || '用于按年龄处理画面特征；留空时不推测年龄。')}</p>
  </form>
}
