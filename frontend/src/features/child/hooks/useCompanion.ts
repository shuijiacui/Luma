...
export function useCompanion(options: Options) {
  ...
  const ask = useCallback(async (utterance: string, requestDrawing = false, feedback: { speak?: boolean; inferDrawingIntent?: boolean; takeTurn?: boolean; commitDrawing?: boolean } = {}) => {
    ...
    try {
      ...
      if (proposed) {
        ...
        if (prepared) {
          const plan = { id: randomId(), revision, proposal: prepared[0], additions: prepared.slice(1), alternatives: cached, aspect }
  ...
  const chooseRecovery = useCallback((id: RecoveryIdea['id']) => {
    ...
    show({ id: randomId(), revision: recovery.revision, aspect: recovery.aspect,
  ...
  const edit = useCallback((patch: Partial<DrawingProposal>, speak = false) => {
    ...
    setProjection({ ...p, id: randomId(), proposal: main, additions: additions as DrawingProposal[], alternatives: [] })
