...
export function LanguageSwitcher() {
  ...
  return (
    <button type="button" onClick={() => setLocale(locale === 'zh' ? 'en' : 'zh')}
      ...
      <span className="luma-language-labels" aria-hidden="true">
        <span lang="zh-CN" className={locale === 'zh' ? 'is-current' : ''}>中文</span>
        <span>/</span>
        <span lang="en" className={locale === 'en' ? 'is-current' : ''}>EN</span>
      </span>
      <span className="luma-language-short" lang={locale === 'zh' ? 'en' : 'zh-CN'} aria-hidden="true">{locale === 'zh' ? 'EN' : '中文'}</span>
    </button>
  )
}
