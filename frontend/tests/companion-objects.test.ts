...
const proposal:DrawingProposal={template:'custom',subject:'松鼠',contribution:'object',recipeId:'squirrel-0',sketch:getDrawingRecipe('squirrel-0')!.sketch,
...
test.each(drawingRecipes.filter(r=>r.id.endsWith('-0')))('voice can select $subject and cycle all three drawings',recipe=>{
 ...
 let current=p
 for(let i=1;i<=3;i++){
  const next=changeVariant(current)
  expect(next?.recipeId).toBe(`${recipe.subject}-${i%3}`)
  current=next!
 }
 expect(current.sketch).toEqual(p.sketch)
})
