...
test('drawing modes and bottom voice settings preserve the active canvas across languages', async () => {
  ...
  try {
    ...
    expect((screen.getByRole('button', { name: '和 Nilo 说话' }) as HTMLButtonElement).disabled).toBe(false)
    expect(screen.queryByRole('button', { name: '声音选项' })).toBeNull()
    expect(screen.getByRole('button', { name: '开启连续对话' })).toBeTruthy()
    ...
    expect((screen.getByRole('button', { name: 'Your turn, Nilo' }) as HTMLButtonElement).disabled).toBe(true)
...
test('templates localize counts and labels while preserving names and unknown user content', () => {
  ...
  expect(t('稳定')).toBe('Steady')
  expect(t('近期情绪较为平稳')).toBe('Recent mood appears steady')
  expect(t('3 幅')).toBe('3 drawings')
  expect(t('管理家庭空间、邀请孩子，也照顾好你的账号。')).toBe('Manage your family, invite children and take care of your account.')
  expect(t('把一段时间里的创作收在一起，回看主题和陪伴建议。')).toBe('Bring recent creations together to revisit themes and companion suggestions.')
  expect(t('从创作观察出发，找一个和孩子开口的温柔理由。')).toBe('Start with their creations and find a gentle way to begin a conversation.')
  expect(t('返回创作记录')).toBe('Back to artwork')
  expect(t('本周成长摘要')).toBe("This week's growth summary")
  expect(t('查看详情')).toBe('View details')
  expect(t('情绪趋势')).toBe('Emotional trend')
  expect(t('本月报告')).toBe('Monthly report')
  expect(t('已生成')).toBe('Ready')
  setLocale('zh')
