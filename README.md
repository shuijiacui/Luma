...
评分采用启发式公式：`w = min(strength × reliability, 0.9)`，同簇取最大值，再合成 `E = 1 − ∏(1 − w_cluster)`，最后由配置参数映射为参考分值。默认展示上限为 `0.85`，不代表准确率。模型自报置信度只参与特征门控。公式、调度与输出语义统一见 [AI 解读与知识库](docs/AI解读与知识库.md)。
...
docs/        功能流程、技术契约、部署验收与研究依据
```

从 [文档导航](docs/README.md) 开始。当前能力、操作流程和限制统一见 [功能与使用流程](docs/功能与使用流程.md)；研究来源与适用局限见 [研究依据](docs/研究依据.md)。已被实现替代的阶段计划和重复记录可从 Git 历史查阅。
