...
| Nilo 共创 | 自己画 / 一起画、相关小物件组合、画布投影、整组颜色/大小/位置调整、明确确认、分别撤销 | 21 种受约束笔迹模板，每轮最多 4 个相关元素；参照已确认画面与主体比例，局部避让；语义质量仍需真实画作评测 |
...
配置与验收：[Nilo API / 语音服务](server/NILO_API.md) · [共创交付与测试说明](docs/Nilo共创.md)。默认示例使用阿里百炼 Qwen3-ASR-Flash + Qwen3-TTS-Flash，音色为支持中英文的 Mochi（沙小弥）童真男声。密钥仅配置在服务端；未配置密钥时不调用付费云语音。浏览器降级优先同语种的年轻、温和音色，实际效果取决于设备，不能保证提供童声。
