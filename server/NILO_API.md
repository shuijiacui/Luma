...
### 当前点击接力协议 v2（2026-09-21）

儿童端点击发送 `takeTurn: true, requestDrawing: true, drawingProtocol: 2, turnScope: "scene"`。普通语音及未带版本的旧客户端保留 v1。下方通用 API 示例和旧部件说明不代表 v2 的模型输出契约。
...
- v2 返回 `protocolVersion:2` 和 `drawingMetrics:{plans,compiled,preflight,reviews,repairs}`。这些是执行计数，不是语义评分。失败保留原因并走现有本地可选画法，孩子确认才提交；不能把这类结果计入 AI 共创成功率。
...
若多笔画共创未能落笔，前端提供本地叶子/云朵选择，先投影、后确认；继承当前画笔，沿用真实碰撞、版本、取消和整组撤销检查。选择不增加模型调用，也不自动打开语音。它是明确点选的基础画法，不是模型识别成功的证据。
