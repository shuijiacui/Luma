...
所有路径以 `/api/nilo` 开头。绘画与语音允许游客与儿童账号，家长账号返回 403；公开素材审核状态接口不限制角色。

## 绘本插画参考与素材审核

素材库含 330 份审核可用矢量画法和 368 份独立 PNG 插画；另有 178 份历史矢量记录仅用于恢复旧文档。新增 334 张 PNG 为 167 个具体题材各提供一张初级和一张中级参考。默认 v3 共创可在描摹模式选择 `illustrationId`，与 `recipeId`、`sketch` 互斥。响应使用 `template: illustration`、可信 `illustrationId`、登记主题名称和常规位置尺寸字段；不能传 URL、图片数据或 SVG。插画尺寸上限为 0.9 × 0.9，整张旋转后须在画布内，且一轮只放一张插画。旧矢量规则不变。

插画是可移动、缩放、清除的浅灰参考底图，允许查看原图，与笔迹保存和导出分离。草稿只保存 ID 与摆放数据。模型根据画面和孩子的自然语言选择，不向孩子展示专业风格菜单。旧 v1/v2 仍使用矢量；默认 v3 和可选 idea-first 的普通同题材请求可选择 PNG，明确单物体请求也可走本地插画计划。复杂组合仍保留自定义绘画路径。

`GET /api/nilo/materials/curation` 返回 `{version:1,decisions:{id:"keep"|"reject"}}`，禁止缓存，不含儿童数据。服务器每轮选材读取项目最新审核文件；前端换画法前同步状态，失败时保留已知记录。下架项退出新选材，已有参考底图仍可恢复和编辑。审核写入仅由本机审阅服务完成，见 [使用说明](../knowledge/nilo/material-curation.md)。
...
模型从审核通过的矢量和 PNG 素材中选择，也可为目录外事物生成至多 24 条路径／96 条命令。风格名仅供内部选图：孩子点名事物即可，模型结合已有画面自动选画法。“圆一点”“细节多一点”“像真的一样”可调整当前底图，明确偏好优先，不能把写实要求静默替换为可爱版本。`backupRecipeId` 由模型选择，不是程序随机替代。只剩极少简单线段的自定义结果不当作完整物体；使用模型选择的相关画法或纠正数据。支持有界纯数据命令字符串的无损解析，不执行 SVG/代码。

矢量目录按具体题材紧凑列出，仅当同题材六个变体全部可用时使用 @6 索引；下架和删除的编号不会被展开。PNG 目录只向模型提供选材必需的编号、主题和难度字段，路径与图片数据留在程序内。扩充不增加默认模型调用轮次。空白画布上的明确点名请求可直接选取本地画法；已有笔迹且没有明确画法偏好时交给正常视觉规划。
...
新画法投影为灰色参考底图（矢量为虚线、PNG 为浅灰图像），孩子可缩放、移动并自己描画，按清除按钮移除底图；不会自动变成孩子的笔迹。取消、版本变化、保存与语音编辑继续适用。断网或连续格式失败仍报告服务问题，不伪称模型已完成画作。
...
- drawing.recipeId 可选择 shared/niloRecipes.mjs 的 330 份审核可用原创矢量画法；程序二次查表取得笔迹，不把整套路径塞入模型提示。模型也可输出不在库中的自定义完整物体（24 paths/96 commands）；部件仍受 8 paths 和接点限制。
...
规划输出可带 `color: "#RRGGBB"`，备选方案独立带颜色；程序保留该颜色，同时继承儿童笔刷和线宽。有效颜色不会再被儿童当前颜色覆盖；未提供有效颜色时使用所选原创画法的建议色。目录按物体归组，只包含审核可用的变体 ID。

绘本风画法以 `[cute]` 标记，检索依次考虑题材相关性、近期使用记录、绘本风偏好。描摹模式的明确单一题材语音请求可直接使用对应画法；组合、特殊姿态与多分句仍交给模型。新素材短边以 144 像素为可读尺寸目标，小画布受边界和面积上限约束。前端仍将其显示为可移动、缩放、清除的灰色虚线底图。
...
服务端 `NILO_CREATIVE_MODE=idea-first` 开启，省略或设为 `catalogue` 沿用原流程。第一轮 `nilo_creative_ideate` 看画面和对话，自由决定主意、关系与必需细节，不接收画法/课程目录；本地按主意检索最多 6 个相关画法；第二轮 `nilo_creative_render` 选择完整匹配的画法或输出自定义路径。需要特殊组合的主意必须自绘，可以参考候选路径；无匹配时为空候选。最多一次同主意修复，不允许用另一个物体降级，沿用总截止时间和取消逻辑。
