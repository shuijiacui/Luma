...
export function deleteFamily(db, familyId, uploadDir) {
  ...
  try {
    ...
    db.prepare('DELETE FROM parent_conversation_turns WHERE child_id IN (SELECT id FROM accounts WHERE family_id = ?)').run(familyId)
    db.prepare('DELETE FROM parent_conversations WHERE child_id IN (SELECT id FROM accounts WHERE family_id = ?)').run(familyId)
