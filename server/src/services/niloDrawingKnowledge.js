...
export function drawingKnowledgeLibrary() {
  if (!cached) {
    const techniques = JSON.parse(readFileSync(new URL('lessons/drawing-techniques.json', root)))
    cached = {
      cards: JSON.parse(readFileSync(new URL('lessons/drawing-knowledge.json', root))).cards,
      records: JSON.parse(readFileSync(new URL('references/quickdraw/drawing-library.json', root))).records,
...
export function sanitizeKnowledgeObservation(raw) {
  ...
  const subjects=Array.isArray(raw?.subjects)?raw.subjects.slice(0,4).flatMap(s=>{
    ...
    const inputs=Array.isArray(s.regions)?s.regions:Array.isArray(raw.regions)?raw.regions.slice(0,8).filter(r=>{
      const owners=raw.subjects.slice(0,4).filter(subject=>contains(box(subject.bounds),box(r?.bounds)))
...
function renderReferenceSheet(records) {
  ...
  return {imageBase64:PNG.sync.write(png).toString('base64'),description:`RETRIEVED DRAWING REFERENCES ONLY, not the child's canvas. Read row by row, left to right, two columns: ${records.map((r,i)=>`${i+1}=${r.category}`).join('; ')}. Blank cells are empty. Quick, Draw! contributors, CC BY 4.0; scaled/recoloured examples. These WHOLE drawings illustrate simple stroke vocabulary and part relationships, NOT objects already in the child canvas. Use a relevant complete NEW object or a missing detail. Whole-object recipes are retrieved separately by their IDs. Do not copy sample coordinates into the child canvas. Recheck the original image before selecting an idea.`}
}
