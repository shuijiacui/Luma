...
export const NILO_TEMPLATES = ['waves', 'fish', 'leaf', 'window', 'stars', 'cloud', 'flower', 'trail', 'flame', 'rain', 'grass', 'echo', 'sun', 'moon', 'tree', 'mountain', 'house', 'boat', 'bird', 'butterfly', 'heart']
...
export const NILO_DIALOGUE_MAX_TOKENS = 1800
...
export function sanitizeDialogueContext(input = {}) {
  const templates = list => Array.isArray(list) ? [...new Set(list.filter(item => NILO_TEMPLATES.includes(item)))].slice(-12) : []
  ...
  return {
    ...
    currentProposal,
    currentAdditions: currentAdditions.every(Boolean) ? currentAdditions : [],
    drawingStyle,
    canvasSize,
...
The preview will be checked locally and needs the child's explicit confirmation AS A GROUP. Describe the whole proposal briefly as a preview, never a finished change. additions is an array of at most 3 further proposal objects. Omit additions when unnecessary. You may provide at most one alternative with a different supported template ONLY for a single-element preview; never combine alternatives with additions. Never output arbitrary paths, HTML, code, commands or extra fields.
...
export function validateProposal(raw, context = {}) {
  ...
  if (context.rejectedTemplates?.includes(raw.template) && !explicitlyRequests(raw.template, context.utterance)) return null
  const allowed = new Set(['template', 'x', 'y', 'width', 'height', 'rotation', 'color', 'strokeWidth', 'brushKind', 'target', 'relation', 'echoPoints', 'anchor', 'placement'])
  ...
  return { template: raw.template, x, y, width, height, rotation, color: color.toLowerCase(), strokeWidth: finalWidth, ...(brushKind ? { brushKind } : {}), target, relation, ...(echoPoints ? { echoPoints } : {}), ...(anchor ? { anchor } : {}), ...(raw.placement ? { placement: raw.placement } : {}) }
}
...
export function validateDialogue(raw, context, hasImage) {
  ...
  if (raw.proposal != null) {
    ...
    if ([proposal, ...additions].reduce((area, item) => area + item.width * item.height, 0) > 0.24) return null
    result.proposal = proposal
    if (additions.length) result.additions = additions
    const alternative = !additions.length && Array.isArray(raw.alternatives) ? raw.alternatives.slice(0, 1).map(item => validateProposal(item, context)).find(item => item && item.template !== proposal.template && (item.template !== 'echo' || context.lastStroke?.points?.length)) : null
...
export async function generateNiloDialogue({ imageBase64, context: input, chatWithImage = defaultVision, chatText = defaultText, timeoutMs, signal } = {}) {
  ...
  try {
    ...
    const opts = { signal: controller.signal, maxTokens: NILO_DIALOGUE_MAX_TOKENS, retries: 0, privateContent: true, disableThinking: true, requireFinalContent: true, kind: hasImage ? 'nilo_companion_vision' : 'nilo_companion_text', responseFormat: { type: 'json_object' } }
