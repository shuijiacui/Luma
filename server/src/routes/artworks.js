...
import { canvasProvenance, validateCanvasDocument } from '../services/canvasDocument.js'

const summary = row => ({ id: row.id, revision: row.revision, createdAt: row.created_at, updatedAt: row.updated_at,
  provenance: row.provenance ?? 'unknown', imageUrl: `/api/artworks/${row.id}/image` })
...
export function createArtworkRouter({ db }) {
  ...
  router.get('/', (req, res) => {
    ...
    const rows = db.prepare('SELECT id, revision, created_at, updated_at, provenance FROM artworks WHERE child_id = ? ORDER BY updated_at DESC, id DESC LIMIT 25 OFFSET ?').all(req.auth.accountId, offset)
  ...
  router.get('/:id', (req, res) => {
    ...
    res.json({ ...summary(row), image: `data:image/png;base64,${row.image_base64}`, ...(row.document_json && { document: JSON.parse(row.document_json) }) })
  })
  ...
  router.put('/:id', (req, res) => {
    const { image, revision, document } = req.body ?? {}
    if (document !== undefined && !validateCanvasDocument(document)) return res.status(400).json({ error: 'invalid canvas document' })
    const documentJson = document ? JSON.stringify(document) : null
    const provenance = canvasProvenance(document)
    ...
    if (previous?.image_base64 === encoded && previous.document_json === documentJson) return res.json(summary(previous))
    if ((previous?.revision ?? 0) !== revision) return res.status(409).json({ error: '这幅画已在其他页面更新，请先下载当前画作，再从历史图画重新打开。' })
    const now = new Date().toISOString()
    db.prepare(`INSERT INTO artworks (id, child_id, image_base64, document_json, provenance, revision, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, 1, ?, ?) ON CONFLICT(id) DO UPDATE SET image_base64 = excluded.image_base64,
      document_json = excluded.document_json, provenance = excluded.provenance,
      revision = artworks.revision + 1, updated_at = excluded.updated_at`).run(req.params.id, req.auth.accountId, encoded, documentJson, provenance, now, now)
